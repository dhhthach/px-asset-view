...
function demoGetChildren(node, isGetNextChildren) {
  ...
  if (node.data.total !== 0) {
    deferred.resolve(children.slice(0, this.itemsPerLevel));
  } else {
    deferred.resolve([]);
  }
...
document.addEventListener("WebComponentsReady", function() {
  var dendrogram = document.querySelector('#px_dendrogram_1');
  dendrogram.addEventListener('px-dendrogram-children-request', function(e) {
...
function runBaseTests() {
  px_dendrogram_1 = document.getElementById('px_dendrogram_1');

  suite('Base Automation Tests for px-dendrogram', function() {

    test('Polymer exists', function() {
      assert.isTrue(Polymer !== null);
    });
    test('px-dendrogram fixture is created', function() {
      assert.isTrue(document.getElementById('px_dendrogram_1') !== null);
    });
